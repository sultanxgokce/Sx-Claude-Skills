...
Onaylanan başlık+açıklamayı projenin defter-yazma-yoluyla kaydet. **Nexus:** `/katip` skill'ini çağır
(`scripts/katip.py add "<verbatim>" --ozet "<başlık>" ...`) — bu HEM UI defter-ekranına HEM ajan-plan-defterine
tek-yazımla yansır (aynı `katip-defter.jsonl`, ayrı-eşleme yok). Proje `/katip` benzeri bir yazıcı sunmuyorsa,
projenin kendi defter/inbox-yazma-yoluna eşle (PRENSİP: onaylı-görev → kalıcı-diske-yaz). Kaydettikten sonra
tek-satır teyit: "📒 kaydedildi: <id> · <başlık>". Sultan'a değer-döndür, sessizce yazıp geçme.
