...
const t=await pw.chromium.launch({args:['--no-sandbox','--force-color-profile=srgb']});
...
await s.addStyleTag({content:'.sahne text,.sahne .gun,.sahne .sentetik,.sahne .okuma,.sahne .dikey *,.sahne .dugme,.sahne h1{color:transparent!important;fill:transparent!important}'});
...
son.sort((a,b)=>a.oran-b.oran);toplamMetin+=son.length;ihlal+=son.filter(o=>o.oran<5.5).length+son.filter(o=>o.tepe<4.5).length;console.log(`${en} ${cipa} · ${son.length} metin · en düşük:`,son.slice(0,5).map(o=>`${o.m}=${o.oran} (en açık piksel ${o.tepe})`).join(' | '),'· 5,5 altı (p95):',son.filter(o=>o.oran<5.5).length,'· 4,5 altı (en açık piksel):',son.filter(o=>o.tepe<4.5).length);
await s.close();}
await t.close();
if (toplamMetin === 0) {
