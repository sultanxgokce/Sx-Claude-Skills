...
- **Dağıtım:** elle-apply (`sync-skills.mjs --apply`, bilinçli) — `_global` (ortak-mount: kod/pc/vekatip/mmex/medigate) VEYA per-proje. **huma = curated-köprü** (`_global` ULAŞMAZ). Otomatik-tetikleyici YOK (elle disiplin).
