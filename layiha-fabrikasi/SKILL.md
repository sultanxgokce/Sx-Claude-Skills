...
katmanı (`scripts/layiha-aday-havuzu.sh`) ve terfi-hedefi (`layiha` skill'i, defter) FABRIKA'nın
...
| **Fabrikayı kapat** (hata bulundu, üretimi durdur) | `bash scripts/layiha-fabrika.sh kapat --sebep "<neden>"` |
| **Fabrikayı aç** (düzeltme bitti, üretime devam) | `bash scripts/layiha-fabrika.sh ac` |
| **Durum bak** (açık mı kapalı mı, neden) | `bash scripts/layiha-fabrika.sh durum` |
| **Eldeki adayları görüntüle/terfi et** (HER-ZAMAN çalışır — kill-switch'ten ETKİLENMEZ) | `bash scripts/layiha-aday-havuzu.sh liste` · `goster <id>` · `terfi <id>` · `durum` |

## Kill-switch (FAZ-D2, `_agents/spec/layiha-fabrikasi-dagitim-DESIGN.md` §5)

- Bayrak: `/config/.claude/layiha-fabrikasi.kapali` (10/10 container'da AYNI-ANDA etkili — paylaşılan
  `_global` bind-mount, DESIGN §1.1/1.2). Yoksa=açık · varsa=kapalı. İçerik opsiyonel (boş de yeterli;
  varsa gerekçe/tarih `durum`'da gösterilir).
- **Kapsam DAR (bilinçli):** kill-switch YALNIZ **layiha üretim-bandına** girer —
  `scripts/mucit-t1.sh --profil layiha` (bandın zorunlu boğaz-noktası). MUAF olanlar:
  (1) `scripts/layiha-aday-havuzu.sh` (liste/göster/terfi/durum) — Sultan'ın eldeki aday-havuzuna
  erişimi asla kesilmez; (2) **DİVAN fikir-hattı** — `mucit-t1.sh --profil divan` (VARSAYILAN) ve
  `scripts/kasif-havuz-ekle.sh`. DİVAN kendi anayasası (§8) olan AYRI bir programdır;
  "layiha-fabrikasını kapat" onu susturma yetkisi VERMEZ. Kanıt: `scripts/mucit-t1.test.sh` G13-G15.
